...
function getSectionTitle($section_id=null){
    if($section_id == null || $section_id === "all"){
        return "Vision Trade International";
    }
...
function sendSMS($text = "", $numbers_csv_string) {
    try {
        ...
        $api_key = config('custom_variable.sms_configuration.api_key');
        $senderID = config('custom_variable.sms_configuration.sender_id');
        ...
        // Execute cURL and handle potential errors
        ...
        if (empty($sendstatus)) {
            throw new Exception("Invalid SMS API response.");
        }
        ...
        return isset($response['response_code']) ? getModifiedResponseCode($response['response_code']) : 1440;
...
function getModifiedResponseCode($code)
{
    return (int) $code === 202 ? 1101 : 1440;
}
