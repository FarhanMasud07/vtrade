...
use App\Http\Resources\SalesInfoResource;
...
class SaleController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('sales_invoice.index', compact('sales'));
    }
    ...
    public function result(Request $request)
    {
        ...
        return view('sales_invoice.salesresult', compact('sales', 'request'));
    }
    ...
    public function create()
    {
        ...
        $customers = User::where('user_type', 'pos')
            ->select('id', 'name', 'phone', 'address', 'pricedata')
            ->get();
        $products = Product::with('size')
            ->where('discontinued', false)
            ->select('id', 'product_name', 'size_id', 'tp')
            ->get();
        $g_opt = Cache::rememberForever('g_opt', function () {
            return GeneralOption::first();
        });
        $featured_product_ids = json_decode($g_opt->options, true)['featured_product_for_invoice'] ?? [];
        return view('sales_invoice.form', compact('method', 'id', 'customers', 'products', 'featured_product_ids'));
    }
    ...
    public function store(Request $request)
    {
        ...
        foreach ($products as $item) {
            ...
            $total = $qty * $price;
            if ($item['free'] > 0) {
                $productTextFoMail .= "<li>{$item['product_name']} = {$qty} x {$price} + free= {$item['free']}  = {$total}</li>";
            } else {
                $productTextFoMail .= "<li>{$item['product_name']} = {$qty} x {$price}  = {$total}</li>";
            }
        ...
        // Start Store process
        $sale = new Sale;
        $sale->user_id = $request->customer['id'];
        ...
        return response()->json(['redirect_url' => route('sales_invoice.show', $sale->id)]);
    }
    ...
    public function show($id)
    {
        ...
        return view('sales_invoice.show', compact('sale', 'signature', 'delivered_by'));
    }
    ...
    {
        $method = "put";
        $sales_info = Sale::with('product.size','user')->findOrFail($id);
        $sales_info  = new SalesInfoResource($sales_info);
        $customers = User::where('user_type', 'pos')
            ->select('id', 'name', 'phone', 'address', 'pricedata')
            ->get();
        $products = Product::with('size')
        ...
        $featured_product_ids = json_decode($g_opt->options, true)['featured_product_for_invoice'] ?? [];
        return view('sales_invoice.form', compact('method', 'id', 'customers', 'products', 'g_opt', 'featured_product_ids','sales_info'));
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        $sale = Sale::with('product.size','user')->findOrFail($id);
        try {
            DB::beginTransaction();
            ...
            //for new product
            foreach ($products as $item) {
                $product_info[] = ['sale_id' => $sale->id, 'product_id' => $item['product_id'], 'qty' => $item['qty'], 'free' => $item['free'], 'price' => $item['price'], 'user_id' => $request->customer['id'], 'sales_at' => $request->sales_date . " " . Carbon::now()->toTimeString()];
                $qty = round($item['qty']);
                $price = round($item['price']);
                $total = $qty * $price;
                if ($item['free'] > 0) {
                    $newProductTextForEmail .= "<li>{$item['product_name']} = {$qty} x {$price} + free= {$item['free']}  = {$total}</li>";
                } else {
                    $newProductTextForEmail .= "<li>{$item['product_name']} = {$qty} x {$price}  = {$total}</li>";
                }
                $subTotal = $subTotal + $total;
                $amount[] = $qty * $price;
            }
    ...
    public function invoice(Request $request, $id)
    {
        ...
        $pdf = PDF::loadView('sales_invoice.invoice', compact('sale', 'current_user', 'general_opt_value', 'signature'));
            ...
            if ($sale->sales_at->diffInDays(now()) < $max_days_delay_for_allowing_sms) {
                ...
                if ($g_opt_value['customer_sales_invoice_edited_notify'] == true) {
                    if ($sale->edited == true && $sale->cust_sms == true) {
                        //SMS For Customer
                        $customer_sms_text = "Your invoice ID#" . $sale->id . " is little bit changed." . $customer_product_text . " your current payable amount: " . round($sale->amount) . $condition_booking_text . ". Prepared By: " . $sale->provided_by . "(" . $this->company_name . ")";
                    } elseif ($g_opt_value['customer_sales_invoice_created_notify'] == true) {
                        $customer_sms_text = "Your invoice ID#" . $sale->id . " is created " . $customer_product_text . ". your total payable amount: " . round($sale->amount) . $condition_booking_text . ". Prepared By: " . $sale->provided_by . "(" . $this->company_name . ")";
        ...
        if ($g_opt_value['admin_sales_invoice_created_notify'] == true && $is_send_sms == true) {
            ...
            if ($sale->user->phone != null) {
                ...
                if ($request->deliverymode === "courier") {
                    ...
                    $text =  "Your product has been booked on the " . $request->courier_name . " CN number : " . $request->cn_number . " " . $condition_booking_text . ", invoice date: " . $sale->sales_at->format('d-M-Y') . " net amount:" . $sale->amount . " - ". $this->company_name;


                } else {
                    $text = "Your product has been delivered to you on the " . $request->deliverymode . " invoice date:" . $sale->sales_at->format('d-M-Y') . " net amount:" . $sale->amount . " - ". $this->company_name;
                }
